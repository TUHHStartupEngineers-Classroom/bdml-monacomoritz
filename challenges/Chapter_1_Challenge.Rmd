...
```{r}
# STOCK PRICES
sp_500_prices_tbl <- read_rds("~/Documents/GitHub/data/Business Decisions with Machine Learning/sp_500_prices_tbl.rds")
sp_500_prices_tbl
```
...
```{r}
# SECTOR INFORMATION
sp_500_index_tbl <- read_rds("~/Documents/GitHub/data/Business Decisions with Machine Learning/sp_500_index_tbl.rds")
sp_500_index_tbl
```
...
```{r}
sp_500_daily_returns_tbl <- read_rds("~/Documents/GitHub/data/Business Decisions with Machine Learning/sp_500_daily_returns_tbl.rds")
...
We're going to import the correct results first (just in case you were not able to complete the last step).
```{r}
stock_date_matrix_tbl <- read_rds("~/Documents/GitHub/data/Business Decisions with Machine Learning/stock_date_matrix_tbl.rds")
```
...
We're going to import the correct results first (just in case you were not able to complete the last step).
```{r}
k_means_mapped_tbl <- read_rds("~/Documents/GitHub/data/Business Decisions with Machine Learning/k_means_mapped_tbl.rds")
```
...
umap_results_tbl %>%
    ...
    theme_tq()+labs(title = "Umap Protection")
```
...
```{r}
k_means_mapped_tbl <- read_rds("~/Documents/GitHub/data/Business Decisions with Machine Learning/k_means_mapped_tbl.rds")
umap_results_tbl   <- read_rds("~/Documents/GitHub/data/Business Decisions with Machine Learning/umap_results_tbl.rds")
```
...
newtibble <- sp_500_index_tbl %>%
  select(symbol, company, sector)

umap_kmeans_results_tbl <- umap_kmeans_3_results_tbl %>% 
  left_join(newtibble)
...
umap_kmeans_results_tbl %>%
  ...
  # Formatting
  scale_color_manual(values=c("#2d72d6", "#2dc6d6", "#2dd692", "#2dd792", "#E926DC", "#0B28B7", "#F0E430", "#DC420A", "#9F1DD7", "#27550C")) +
  labs(title = "2D Projection",
       subtitle = "UMAP 2D Projection with K-Means Cluster Assignment",
       caption = "Conclusion: 10 Segments identified using 2 algorithms") +
